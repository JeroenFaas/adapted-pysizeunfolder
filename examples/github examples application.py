...
for K in range(len(shape)):
    reference_sample.append(pickle.load(open(f"examples/{shape[K]}_sample.pkl", "rb")))
...
# Translate range of number of vertices:
...
# reference particle K = 0: tetrahedron; 1: cube; 2: dodecahedron
K = 0

################# Method AV - Interpretation 1 ################
...
errors_biased = pu.error_estimate(x_size_true, y_true_biased, np.array([x_pts_size, x_pts_size, x_pts_size]),
                                  np.array([y_pts_biased_A, y_pts_biased_AV1, y_pts_biased_AV2]))
errors = pu.error_estimate(x_size_true, y_true, np.array([x_pts_size, x_pts_size, x_pts_size]),
                           np.array([y_pts_A, y_pts_AV1, y_pts_AV2]))

print(f"""Estimate errors using reference particle {shape[K]}:
Biased A\t{errors_biased[0]:.4f}
True A\t{errors[0]:.4f}
Biased AV(1)\t{errors_biased[1]:.4f}
True AV(1)\t{errors[1]:.4f}
Biased AV(2)\t{errors_biased[2]:.4f}
True AV(2)\t{errors[2]:.4f}""")

...
def gcm_slope(rawslope, dx):
    ...
    if np.sum(valid_input) == 0:
        # Return zeroes if there are no valid entries in the input.
        return slope, np.array([0])

    slope[valid_input] = IsotonicRegression().fit_transform(np.arange(len(dx))[valid_input], rawslope[valid_input],
                                                            sample_weight=dx[valid_input])
    slope[slope < 0] = 0.0
...
def hybrid_icm_em(a_matrix, estimate_init=None, tol=0.0001, stop_iterations=10):
    ...
    while stop_counter < stop_iterations:
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            rawslope = estimate - first_derivatives/second_derivatives  # Ignoring invalid divisions for now.
